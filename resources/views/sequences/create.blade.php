...
@section('content')
    <div class="pilar-page">
        ...
        <div class="mb-4">
            <h3 class="card-title">Nueva Secuencia</h3>
        </div>
        <div>
            <form method="POST" action="{{ route('sequences.store') }}" class="pilar-form">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <input name="repetitions" class="form-control" type="number" min="1" value="1">
                </div>
                <div class="form-group">
    ...
    <script>
        ...
        function makeRow(idx, selectedId){
            ...
            row.innerHTML = `
                <div class="col-6">
                    <select name="items[${idx}][bell_sound_id]" class="form-control">
                        ${sounds.map(s=>`<option value="${s.id}">${s.nombre}</option>`).join('')}
                    </select>
                </div>
                <div class="col-3">
                    <input name="items[${idx}][interval_seconds]" value="1" class="form-control" type="number" step="0.01" min="0" title="Segundos desde el inicio del item anterior">
                </div>
                <div class="col-2">
                    <input name="items[${idx}][orden]" value="${idx + 1}" min="1" class="form-control" type="number">
                </div>
                <div class="col-1"><button type="button" class="btn btn-danger btn-sm remove">-</button></div>
            `;
            if(selectedId){ const sel = row.querySelector('select'); if(sel) sel.value = selectedId; }
        ...
        // preview current unsaved sequence
        ...
        document.getElementById('previewSequence')?.addEventListener('click', async ()=>{
            ...
            // sort by user-provided orden (numeric, ascending)
        ...
        function updateDurationDisplay(){
            ...
            document.getElementById('sequenceDuration').textContent = formatSeconds(finalDuration) + ' (' + repetitions + 'x)';
        }
        // update when items or repetitions change
